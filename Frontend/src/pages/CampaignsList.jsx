...
const CampaignsList = () => {
  ...
  const [qrModalId, setQrModalId] = useState(null); // ID of campaign whose QR modal is open
  ...
  const handleDelete = async (id, title) => {
    if (
      !window.confirm(
        `Are you sure you want to delete "${title}"? This will delete all fields and registration submissions forever.`,
      )
    )
      return;
    try {
  ...
  // Download QR from modal (large hidden QR)
  const downloadQRModal = (campId) => {
    const svgEl = document.getElementById(`qr-modal-svg-${campId}`);
    if (!svgEl) return;
    const svgData = new XMLSerializer().serializeToString(svgEl);
  ...
  const qrModalCampaign = campaigns.find((c) => c.id === qrModalId);

  return (
    <div className="flex-1 p-6 space-y-6 max-w-7xl mx-auto w-full overflow-y-auto">

      {/* ── QR Expand Modal ── */}
      {qrModalId && qrModalCampaign && (
        <div
          className="fixed inset-0 z-[9999] flex items-center justify-center p-4"
          onClick={() => { setQrModalId(null); setQrModalCopied(false); }}
        >
          {/* Backdrop */}
          ...
          <div
            ...
            className="relative z-10 bg-white/95 backdrop-blur-xl border border-white/60 rounded-3xl shadow-2xl shadow-slate-900/30 p-8 flex flex-col items-center gap-5 max-w-sm w-full animate-[fadeScaleIn_0.22s_ease-out]"
            style={{ animation: "fadeScaleIn 0.22s cubic-bezier(.22,1,.36,1) both" }}
          >
            {/* Close button */}
            <button
              onClick={() => { setQrModalId(null); setQrModalCopied(false); }}
            ...
            <div className="text-center space-y-0.5">
              <p className="text-[10px] font-bold text-violet-500 uppercase tracking-widest">Scan to Open</p>
              <h3 className="text-base font-extrabold text-slate-800 leading-snug line-clamp-2">
                {qrModalCampaign.title}
              </h3>
            </div>
            ...
            <div className="bg-white p-4 rounded-2xl border-2 border-slate-100 shadow-inner">
              <QRCodeSVG
                id={`qr-modal-svg-${qrModalId}`}
                value={`${window.location.origin}/campaign/${qrModalId}`}
                size={240}
                level="H"
                includeMargin={false}
              />
            ...
            {/* URL display */}
            <div className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-center">
              <p className="text-[10px] text-slate-400 font-semibold mb-0.5">Campaign Link</p>
              <p className="text-[11px] text-violet-600 font-bold break-all leading-snug">
                {`${window.location.origin}/campaign/${qrModalId}`}
              </p>
            </div>
            ...
            <div className="flex items-center gap-3 w-full">
              <button
                onClick={() => {
                  navigator.clipboard.writeText(`${window.location.origin}/campaign/${qrModalId}`);
                  setQrModalCopied(true);
                  setTimeout(() => setQrModalCopied(false), 2500);
                }}
                className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-bold border transition duration-200 ${
                  qrModalCopied
                    ? "bg-emerald-50 border-emerald-400 text-emerald-600"
                    : "bg-violet-50 border-violet-200 text-violet-600 hover:bg-violet-600 hover:text-white hover:border-violet-600"
                }`}
              >
                {qrModalCopied ? <Check className="h-3.5 w-3.5" /> : <Copy className="h-3.5 w-3.5" />}
                {qrModalCopied ? "Copied!" : "Copy Link"}
              </button>
              <button
                onClick={() => downloadQRModal(qrModalId)}
                className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-bold border border-slate-200 bg-slate-50 text-slate-600 hover:bg-slate-700 hover:text-white hover:border-slate-700 transition duration-200"
              >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-5">
        <div className="space-y-1">
          ...
          <p className="text-sm text-slate-500 font-medium">
            Create, configure, monitor, and duplicate dynamic campaign workflows
          </p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="bg-white/70 backdrop-blur-md border border-white/40 py-16 px-6 text-center rounded-2xl flex flex-col items-center">
          ...
          <p className="text-xs text-slate-505 mt-1 max-w-xs leading-relaxed">
            No campaign records match your search or filter rules. Create one to
            get started!
          </p>
        </div>
      ) : (
              ...
              >
                ...
                <div className="h-40 bg-slate-950/20 relative overflow-hidden shrink-0">
                  {hasBanner ? (
                    <img
                      src={
                        camp.banner_url.startsWith("/")
                          ? `${api.BACKEND_URL}${camp.banner_url}`
                          : camp.banner_url
                      }
                      alt={camp.title}
                  ...
                  >
                    <div
                      className="bg-white p-0.5 rounded-lg border border-slate-100/80 cursor-zoom-in group/qr relative"
                      onClick={() => setQrModalId(camp.id)}
                    ...
                    >
                      ...
                      />
                      {/* Zoom hint overlay */}
                      <div className="absolute inset-0 flex items-center justify-center bg-violet-600/80 rounded-lg opacity-0 group-hover/qr:opacity-100 transition-opacity duration-150">
                  ...
                  <span
                    className={`absolute top-2.5 right-2.5 px-2.5 py-0.5 rounded-full text-[10px] font-bold shadow-md ${
